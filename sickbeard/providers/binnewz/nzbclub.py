...
class NZBClub(NZBDownloader):
    ...
    def search(self, filename, minSize, newsgroup=None):
        ...
        for suffixURL in nzbClubURLs:
            ...
            try:
                nzbClubSoup = BeautifulSoup( self.open(nzbClubURL).read().decode('utf-8','ignore'), "html5lib")
            except:
                nzbClubSoup = BeautifulSoup( self.open(nzbClubURL).read().decode('latin-1','ignore'), "html5lib")
            if nzbClubSoup:
                sizeInMegs = None
